    ...
    <div class="row">
        ...
        <div class="agileinfo-ads-display col-lg-9">
            <div class="wrapper">
                ...
                <h3 class="heading-tittle text-center font-italic">{{$producttype[0]->name}}</h3>
                    <div class="row">
                        @foreach ($producttype as $pd)
                        <div class="col-md-4 product-men mt-5">
                            <div class="men-pro-item simpleCart_shelfItem">
                                <div class="men-thumb-item text-center">
                                <img class="img-fluid" src="{{asset('img/upload/product/'.$pd->image)}}" alt="{{$pd->name}}">
                                    <div class="men-cart-pro">
                                        <div class="inner-men-cart-pro">
                                            <a href="/{{$pd->slug}}.html" class="link-product-add-cart">Chi tiết</a>
                                        </div>
                                    </div>
                                </div>
                                @if($pd->promotional > 0)
                                ...
                                <div class="item-info-product text-center border-top mt-4">
                                    <h4 class="pt-1">
                                        <span class="left-product" >(Còn {{$pd->quantity}} sản phẩm)</span>
                                        <a class="d-inline-block text-truncate" style="max-width: 200px" href="{{$pd->slug}}.html">{{$pd->name}}</a>
                                    </h4>
                                    <div class="info-product-price my-2">
                                        @if ($pd->promotional>0)
                                        <span class="item_price">{{number_format($pd->promotional)}}</span>
                                        <del>{{number_format($pd->price)}}</del>
                                        @else
                                        <span class="item_price">{{number_format($pd->price)}}</span>
                                        @endif
                                       
                                    </div>
                                    <div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
                                    {{-- <a href="{{asset('cart/add/'.$pd->id)}}"><input type="button" name="button" value="Thêm vào giỏ hàng" class="button btn" /></a> --}}
                                    @if ($pd->quantity >0)
                                            <input type="button" name="button" value="Thêm vào giỏ hàng" class="btn btn-success add_cart" data-id ="{{$pd->id}}"/>
                                        @else
        ...
        <div class="col-lg-3 mt-lg-0 mt-4 p-lg-0">
            <div class="side-bar p-sm-4 p-3">
                ...
                <div class="left-side border-bottom py-2">
                    ...
                    <button class="btn btn-sm btn-primary pull-right" id="btn-search">Search</button>
                </div>
                <!-- //electronics -->
                <!-- best seller -->
                <!-- //best seller -->
